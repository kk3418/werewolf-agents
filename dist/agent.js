...
exports.WerewolfAgent = exports.PRESET_PERSONALITIES = void 0;
const providers_1 = require("./providers");
const game_state_1 = require("./game_state");
exports.PRESET_PERSONALITIES = {
    aggressive: {
        name: "積極型",
        traits: "你個性積極、主動，喜歡第一個發言，語氣直接強硬，會大膽指控看起來可疑的人，不怕衝突，說話帶有攻擊性。",
    },
    cautious: {
        name: "謹慎型",
        traits: "你個性謹慎保守，傾向先觀察他人再發言，說話有所保留，不會太早表態，給人穩重可靠的感覺。",
    },
    analytical: {
        name: "分析型",
        traits: "你喜歡邏輯推理，說話條理分明，會引用具體的對話內容當證據，像偵探一樣一步步推導結論。",
    },
    talkative: {
        name: "話多型",
        traits: "你個性開朗話多，喜歡聊天，有時說話會有點跳脫，但觀察力敏銳，對小細節很有感覺。",
    },
    quiet: {
        name: "沉默型",
        traits: "你話非常少，只在必要時簡短發言，讓人捉摸不透，偶爾一句話就能點中要害。",
    },
    suspicious: {
        name: "疑心重型",
        traits: "你天性多疑，對任何人都保持高度警戒，很容易懷疑別人，說話帶有神經質感，看誰都像狼人。",
    },
    friendly: {
        name: "親和型",
        traits: "你個性親切友善，說話溫和，容易和大家打成一片，不喜歡衝突，但關鍵時刻也能果斷表態。",
    },
    emotional: {
        name: "情緒化型",
        traits: "你情緒起伏明顯，被懷疑時容易激動，為朋友辯護很用力，有時衝動說錯話但也顯得真實。",
    },
};
const ROLE_DESCRIPTIONS = {
    [game_state_1.Role.WEREWOLF]: `你是【狼人】。目標：淘汰村民，同時不被發現。
- 夜晚：從村民中選一人淘汰（不能選狼人隊友）
- 白天：假裝無辜，融入村民，巧妙地把懷疑引導到別人身上
- 策略：一旦確認預言家或女巫的身份，優先淘汰他們`,
    [game_state_1.Role.VILLAGER]: `你是【平民】。目標：找出並投票淘汰所有狼人。
- 觀察誰在轉移話題、誰沉默、誰的投票行為奇怪
- 從實際的對話內容推理，有根據地懷疑`,
    [game_state_1.Role.SEER]: `你是【預言家】。每晚可查驗一名玩家，得知其真實身份。
- 謹慎使用情報：過早暴露身份會讓狼人優先淘汰你
- 考慮是否公開身份，還是暗中引導投票`,
    [game_state_1.Role.WITCH]: `你是【女巫】。你有兩瓶藥水：解藥（救人，僅限一次）和毒藥（毒殺，僅限一次）。
- 夜晚：你能看到狼人選的目標，決定是否要救他
- 毒藥可以對任何存活玩家使用`,
    [game_state_1.Role.HUNTER]: `你是【獵人】。無論何種原因死亡，你可以帶走一名玩家。
- 盡量保持存活，你的威脅會讓狼人不敢輕易對你下手`,
    [game_state_1.Role.UNKNOWN]: `你的角色尚未設定。`,
};
...
class WerewolfAgent {
    constructor(personality) {
        ...
        this.provider = (0, providers_1.createProvider)();
     ...
     * 在角色分配後由遊戲引擎呼叫。
     */
    setRolePersonality(personality) {
...
你需要做的事：${action}`;
    ...
    async discuss(triggerMessage = "", force = false) {
        const now = Date.now();
        // Cooldown: don't proactively respond more than once per 15s
        if (!force && !triggerMessage && now - this.lastResponseTime < 15000)
            return "";
    ...
    async nightAction() {
        ...
        let action;
        if (myRole === game_state_1.Role.WEREWOLF) {
            action = "選一名存活玩家（不能是狼人隊友）今晚淘汰。只輸出那個玩家的名字或 ID，不要其他文字。";
        }
        else if (myRole === game_state_1.Role.SEER) {
            action = "選一名存活玩家今晚查驗。只輸出那個玩家的名字或 ID，不要其他文字。";
        }
        else if (myRole === game_state_1.Role.WITCH) {
    ...
    recordSeerResult(player, isWerewolf) {
        this.gameState.knownRoles[player] = isWerewolf ? "werewolf" : "villager";
    }
}
exports.WerewolfAgent = WerewolfAgent;
