...
const multi_agent_game_1 = require("./multi_agent_game");
...
async function main() {
    ...
    let personalities = playerNames.map(() => null);
    let rolePersonalities = {};
    ...
    if (hasPlayerPersonalityEnv || hasRolePersonalityEnv) {
        ...
        if (hasPlayerPersonalityEnv) {
            ...
            personalities = playerNames.map((_, i) => {
                const key = keys[i];
                return key && agent_1.PRESET_PERSONALITIES[key] ? agent_1.PRESET_PERSONALITIES[key] : null;
            });
        ...
        if (hasRolePersonalityEnv) {
            ...
            const roleMap = {
                werewolf: game_state_1.Role.WEREWOLF, villager: game_state_1.Role.VILLAGER, seer: game_state_1.Role.SEER,
                witch: game_state_1.Role.WITCH, hunter: game_state_1.Role.HUNTER,
            };
            process.env.ROLE_PERSONALITIES.split(";").forEach((pair) => {
                const [roleKey, personalityKey] = pair.split("=").map((s) => s.trim().toLowerCase());
                const role = roleMap[roleKey];
                const personality = agent_1.PRESET_PERSONALITIES[personalityKey];
                if (role && personality)
                    rolePersonalities[role] = personality;
            });
            console.log("\n角色個性（來自 ROLE_PERSONALITIES）：");
            Object.entries(rolePersonalities).forEach(([role, p]) => {
                console.log(`  【${role}】：${p.name}`);
            });
        }
    }
    else {
        ...
        if (modeInput === "1" || modeInput === "2") {
            console.log("\n可用個性：");
            Object.entries(agent_1.PRESET_PERSONALITIES).forEach(([key, p]) => {
        ...
        if (modeInput === "1") {
            ...
            for (let i = 0; i < playerNames.length; i++) {
                ...
                if (key && agent_1.PRESET_PERSONALITIES[key]) {
                    personalities[i] = agent_1.PRESET_PERSONALITIES[key];
                    console.log(`    ✅ ${agent_1.PRESET_PERSONALITIES[key].name}`);
                }
            }
        }
        else if (modeInput === "2") {
            // Per-role
            const roles = [
                { key: "werewolf", role: game_state_1.Role.WEREWOLF, label: "狼人" },
                { key: "seer", role: game_state_1.Role.SEER, label: "預言家" },
                { key: "witch", role: game_state_1.Role.WITCH, label: "女巫" },
                { key: "hunter", role: game_state_1.Role.HUNTER, label: "獵人" },
                { key: "villager", role: game_state_1.Role.VILLAGER, label: "平民" },
            ];
            ...
            for (const { role, label } of roles) {
                ...
                const key = input.trim().toLowerCase();
                if (key && agent_1.PRESET_PERSONALITIES[key]) {
                    rolePersonalities[role] = agent_1.PRESET_PERSONALITIES[key];
                    console.log(`    ✅ ${agent_1.PRESET_PERSONALITIES[key].name}`);
                }
            }
        }
    ...
    // ── Launch ───────────────────────────────────────────────────────────────
    console.log("\n── 遊戲即將開始 ──\n");
    const game = new multi_agent_game_1.WerewolfMultiAgentGame({
        playerNames,
        personalities,
        rolePersonalities,
        delayMs,
